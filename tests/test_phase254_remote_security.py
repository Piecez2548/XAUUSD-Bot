...
def test_remote_api_surface_is_read_only_and_payloads_are_safe(remote_database: Database) -> None:
    ...
    for route in app.routes:
        ...
        for method in route.methods:
            if method == "POST":
                policy_path = route.path.replace(
                    "{account_id}", "123e4567-e89b-12d3-a456-426614174000"
                ).replace("{session_id}", "123e4567-e89b-12d3-a456-426614174001")
                assert (
                    is_private_control_path_allowed(route.path, method)
                    or is_auth_route_allowed(route.path, method)
                    or is_auth_admin_route_allowed(policy_path, method)
                )
            elif method == "DELETE":
                policy_path = route.path.replace(
                    "{account_id}", "123e4567-e89b-12d3-a456-426614174000"
                ).replace("{session_id}", "123e4567-e89b-12d3-a456-426614174001")
                assert is_auth_admin_route_allowed(policy_path, method)
            else:
