...
export function ShadowPage() {
  ...
  return <div className="page"><PageIntro eyebrow="DETERMINISTIC SHADOW ENGINE" title="Shadow Trading" detail="Hypothetical decisions from completed broker snapshots. No orders are submitted." /><SummaryCards items={[["Total", number(summary.data?.total, 0)], ["BUY", number(summary.data?.BUY, 0)], ["SELL", number(summary.data?.SELL, 0)], ["NO_TRADE", number(summary.data?.NO_TRADE, 0)], ["Pending outcomes", number(summary.data?.pending_outcomes, 0)], ["Execution", "DISABLED"]]} /><Panel title="Operational Liveness" action={<StatusPill state={health.data?.state ?? "UNKNOWN"} />}>{health.data ? <div className="metrics"><div className="metric-row"><span>Worker state</span><strong>{health.data.state}</strong></div><div className="metric-row"><span>Latest available M5</span><strong>{utcTime(health.data.latest_available_m5 ?? health.data.latest_persisted_m5, true)}</strong></div><div className="metric-row"><span>Latest received M5</span><strong>{utcTime(health.data.latest_received_m5 ?? health.data.last_received_candle_at, true)}</strong></div><div className="metric-row"><span>Latest processed M5</span><strong>{utcTime(health.data.latest_processed_m5 ?? health.data.last_processed_candle_at, true)}</strong></div><div className="metric-row"><span>Latest decision M5</span><strong>{utcTime(health.data.latest_decision_m5 ?? health.data.last_decision_at, true)}</strong></div><div className="metric-row"><span>Processing lag</span><strong>{health.data.processing_lag_seconds == null ? unavailable : `${health.data.processing_lag_seconds}s`}</strong></div><div className="metric-row"><span>Queue / deferred / catch-up</span><strong>{number(health.data.queue_depth, 0)}/{number(health.data.queue_capacity, 8)} · {number(health.data.deferred_count, 0)} · {number(health.data.catchup_pending_count, 0)}</strong></div><div className="metric-row"><span>Total backlog</span><strong>{number(health.data.total_backlog, number(health.data.queue_depth, 0))}</strong></div><div className="metric-row"><span>Execution</span><strong>DISABLED</strong></div></div> : <DataState loading={health.loading} error={health.error} emptyTitle="Shadow worker health unavailable" emptyDetail="No authoritative shadow-worker heartbeat is available." />}</Panel><Panel title="Latest Shadow Decision" action={<StatusPill state="DISABLED" label="SHADOW ONLY" />}>{latest.data ? <div className="metrics"><div className="metric-row"><span>Decision / regime</span><strong>{latest.data.decision} · {latest.data.market_regime}</strong></div><div className="metric-row"><span>Entry / SL / TP</span><strong>{number(latest.data.entry_price)} / {number(latest.data.stop_loss)} / {number(latest.data.take_profit)}</strong></div><div className="metric-row"><span>R:R / hypothetical volume</span><strong>{number(latest.data.risk_reward_ratio)} / {number(latest.data.hypothetical_volume)}</strong></div><div className="metric-row"><span>Reason</span><strong>{latest.data.reason_codes.join(", ")}</strong></div><div className="metric-row"><span>Snapshot</span><strong className="mono">{latest.data.market_snapshot_id ?? unavailable}</strong></div><div className="metric-row"><span>Execution</span><strong>DISABLED — NO ORDER SENT</strong></div></div> : <DataState loading={latest.loading} error={latest.error} emptyTitle="No shadow decision" emptyDetail="No completed verified snapshot has been analysed yet." />}</Panel><Panel title="Recent Decisions">{history.data?.length ? <DataTable rows={history.data} columns={columns} rowKey={(row) => row.decision_id} /> : <DataState loading={history.loading} error={history.error} emptyTitle="No decision history" emptyDetail="Shadow decisions will appear after completed broker observations." />}</Panel></div>;
}
